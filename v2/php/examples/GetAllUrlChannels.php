...
class GetAllUrlChannels {
  ...
  public static function run($service, $adClientId, $maxPageSize) {
    ...
    do {
      ...
      $result = $service->accounts_adclients_urlchannels
                        ->listAccountsAdclientsUrlchannels($adClientId,
                                                           $optParams);
      if (!empty($result['urlChannels'])) {
        $urlChannels = $result['urlChannels'];
        foreach ($urlChannels as $urlChannel) {
          printf("URL channel with URI pattern \"%s\" was found.\n",
              $urlChannel['uriPattern']);
        }
