...
namespace AdSense.Sample
{
    ...
    public class ManagementApiConsumer
    {
        ...
        private IList<AdUnit> GetAllAdUnits(string adClientId)
        {
            ...
            do
            {
                ...
                if (!adUnitResponse.AdUnits.IsNullOrEmpty())
                {
                    ...
                    {
                        Console.WriteLine(
                            "Ad unit with ID \"{0}\", name \"{1}\" and state \"{2}\" was found.",
                            adUnit.Name,
                            adUnit.DisplayName,
                            adUnit.State);
                    }
        ...
        private IList<CustomChannel> GetAllCustomChannels(string adClientId)
        {
            ...
            do
            {
                ...
                if (!customChannelResponse.CustomChannels.IsNullOrEmpty())
                {
                    ...
                    {
                        Console.WriteLine(
                            "Custom channel with ID \"{0}\" and name \"{1}\" was found.",
                            customChannel.Name,
                            customChannel.DisplayName);
                    }
                }
                else
                {
        ...
        private void DisplayAllAdUnitsForCustomChannel(string customChannelId)
        {
            ...
            do
            {
                var adUnitRequest = service.Accounts.Adclients.Customchannels.ListLinkedAdUnits(
                    customChannelId);
                adUnitRequest.PageSize = maxListPageSize;
                adUnitRequest.PageToken = pageToken;
                adUnitResponse = adUnitRequest.Execute();
        ...
        private void DisplayAllUrlChannels(string adClientId)
        {
            ...
            do
            {
                var urlChannelRequest = service.Accounts.Adclients.Urlchannels.List(
		    adClientId);
	    ...
	    reportRequest.EndDateDay = 31;
            //reportRequest.Filters = new List<string>
            //{
            //    "AD_CLIENT_ID==" +
        ...
        private IList<SavedReport> GetAllSavedReports()
        {
            ...
            do
            {
                ...
                if (!savedReportResponse.SavedReports.IsNullOrEmpty())
                {
                    ...
                    {
                        Console.WriteLine(
                            "Saved report with ID \"{0}\" and title \"{1}\" was found.",
                            savedReport.Name,
                            savedReport.Title);
                    }
                }
        ...
        private void DisplayAllAlerts()
        {
            ...
            ListAlertsResponse alertsResponse = service.Accounts.Alerts.List(adSenseAccount.Name).Execute();
        ...
        private void DisplayAllPayments()
        {
            ...
            ListPaymentsResponse paymentsResponse = service.Accounts.Payments.List(adSenseAccount.Name).Execute();
            ...
            if (!paymentsResponse.Payments.IsNullOrEmpty())
            {
                ...
                {
                    Console.WriteLine(
                        "Payment with ID \"{0}\" of {1} and date \"{2}\" was found.",
                        payment.Name,
                        payment.Amount,
                        payment.Date);
                }
